...
public:
  SafetyStop() : Node("safety_stop_node"), is_first_msg_(true), state_{State::FREE}, prev_state_{State::FREE}
  {
    ...
    check_timer_ = create_wall_timer(1s, 
        std::bind(&SafetyStop::checkActionServers, this));
...
void checkActionServers()
{
    if (decrease_speed_client_->action_server_is_ready() &&
        increase_speed_client_->action_server_is_ready())
    {
        check_timer_->cancel();
        RCLCPP_INFO(get_logger(), "Joy turbo action servers ready!");
    }
    else
    {
        RCLCPP_WARN(get_logger(), "Waiting for joy turbo action servers...");
    }
}
};
