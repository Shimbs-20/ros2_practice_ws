...
class AStarPlanner(Node):
    def __init__(self):
        ...
        # BUG FIX 1: /goal_pose from RViz2 uses VOLATILE durability.
        # TRANSIENT_LOCAL subscriber + VOLATILE publisher = QoS incompatible.
        # Message silently dropped → pose_callback never fires.
        goal_qos = QoSProfile(
            depth=10,
        ...
        self.path_pub = self.create_publisher(Path, "/a_star/path", pub_qos)
        self.map_pub  = self.create_publisher(OccupancyGrid, "/a_star/map", pub_qos)
    ...
    def pose_callback(self, pose: PoseStamped):
        ...
        # BUG FIX 4: use base_footprint — AMCL publishes map→odom→base_footprint
        try:
        ...
        self.get_logger().info(
            f"A*: ({start_pose.position.x:.2f},{start_pose.position.y:.2f})"
            f" → ({pose.pose.position.x:.2f},{pose.pose.position.y:.2f})")

        result = self.plan(start_pose, pose.pose)

        if result.poses:
            self.get_logger().info(f"A* path: {len(result.poses)} waypoints")
        ...
        while not pending_nodes.empty() and rclpy.ok():
            ...
            for dirx, diry in exploration_directions:
                ...
                if (new_node not in visited_nodes
                        ...
                        and 0 <= self.map_.data[self.pose_to_call(new_node)] < 99):
                    new_node.cost = active_node.cost + 1 + self.map_.data[self.pose_to_call(new_node)]
            ...
            # BUG FIX 3: throttle visualization — was publishing every cell
            # which blocked the executor preventing path_pub from firing
            if cells_expanded % 200 == 0:
